...
$sqlCommand="INSERT into npe(head,subhead,content,publishdate,company_photo) values('$head','$subhead','$content','$publishdate','$new_pic')";
$query=mysqli_query($conn, $sqlCommand) or die(mysqli_error($conn));
      ...
      //$stmt_update=$conn->prepare('INSERT into np (head,subhead,content,company_photo) VALUES ( head=:head,subhead=:subhead,content=:content ,company_photo=:company_photo)');
 
      //$executed=$stmt_update->execute(array(':head'=>$_POST['head'],':subhead'=>$_POST['subhead'],':content'=>$_POST['content'],':company_photo'=>$new_pic));
...
<center>
      ...
      <button class="btn btn-lg btn-primary btn-block" type="submit" name="update">update</button>  <br><br>
            <button class="btn btn-lg btn-primary btn-block"><a href="entertainment.php">back</a></button>  </center>
